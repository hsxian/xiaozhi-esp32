...
void Mp3MusicPlayer::PlayTask(void* arg) {
    ...
    for (const auto& music : player->current_music_list_) {
        if (player->stop_requested_) {
            break;
        }
        ESP_LOGI(TAG, "Playing track: %s %s from %s", music.artist.c_str(), music.name.c_str(), music.url.c_str());
...
void Mp3MusicPlayer::Stop() { stop_requested_ = true; }
void Mp3MusicPlayer::Pause() {
    // 暂停功能可以通过降低音量实现
    auto& board = Board::GetInstance();
    auto audio_codec = board.GetAudioCodec();
    if (audio_codec) {
        audio_codec->SetOutputVolume(0);
    }
}
void Mp3MusicPlayer::Resume() {
    auto& board = Board::GetInstance();
    auto audio_codec = board.GetAudioCodec();
    if (audio_codec) {
        audio_codec->SetOutputVolume(80);  // 恢复到默认音量
    }
}
void Mp3MusicPlayer::PlayInternal(const Music& music) {
    ...
    if (mp3_data_size >= 10 && mp3_buffer[0] == 'I' && mp3_buffer[1] == 'D' &&
        ...
        // 大小是同步安全整数（最高位为0）
        uint32_t tag_size =
            ((uint32_t)(mp3_buffer[6] & 0x7F) << 21) | ((uint32_t)(mp3_buffer[7] & 0x7F) << 14) |
            ...
            if (samples_decoded == -4) {  // ERR_MP3_NEED_MORE
                ...
                if (found) {
                    ESP_LOGW(TAG, "Resynced, skipped %d bytes to find frame at offset %u",
                             (int)(mp3_data_offset - original_offset),
        ...
        if (frame_info.outputSamps > 0 && !stop_requested_) {
            ...
            if (need_rate_conversion || need_channel_conversion) {
                ...
                if (need_rate_conversion) {
                    output_frames =
                        (int)((int64_t)input_frames * codec_output_rate / input_rate + 0.5);
                    if (output_frames < 1) {
                        output_frames = 1;
                    }
                }
                ...
                uint64_t step = need_rate_conversion
                                    ? (((uint64_t)input_rate << 32) / codec_output_rate)
                                    : (1ull << 32);
                uint64_t pos = 0;
                for (int f = 0; f < output_frames; ++f) {
                    ...
                    if (input_channels == 2 && output_channels == 1) {
                        int32_t s0 =
                            ((int32_t)pcm_buffer[idx * 2] + (int32_t)pcm_buffer[idx * 2 + 1]) / 2;
                        int32_t s1 = s0;
                        if (idx + 1 < (size_t)input_frames) {
                            s1 = ((int32_t)pcm_buffer[(idx + 1) * 2] +
                                  (int32_t)pcm_buffer[(idx + 1) * 2 + 1]) /
                                 2;
                        }
