...
private:
    ...
    bool PreparePlayback(const Music& music, AudioCodec*& audio_codec, NetworkInterface*& network,
    ...
    void ConvertPcmIfNeeded(AudioCodec* audio_codec, const MP3FrameInfo& frame_info,
                            const std::vector<int16_t>& pcm_buffer,
                            std::vector<int16_t>& output_pcm, int& output_samples,
                            int& output_channels);
    bool DecodeAndPlayFrame(AudioCodec* audio_codec, HMP3Decoder decoder,
