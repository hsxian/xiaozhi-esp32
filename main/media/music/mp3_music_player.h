...
struct Mp3DataChunk {
    uint8_t* data;
    size_t size;
    bool is_eos;  // End of stream
    bool is_error;
    Mp3DataChunk() : data(nullptr), size(0), is_eos(false), is_error(false) {}
    Mp3DataChunk(uint8_t* d, size_t s) : data(d), size(s), is_eos(false), is_error(false) {}
};
