...
bool Lyrics::Parse(const std::string& lrc_content) {
            ...
            if (result >= 2) {
                ...
                if (*pos == ']') {
                    ...
                    if (!text.empty()) {
                        ...
                        line.text = text;
                        // ESP_LOGI(TAG, "Parsed line: %s, time: %d", line.text.c_str(),
                        // line.time_ms);
    ...
    std::sort(lines_.begin(), lines_.end(),
              [](const LyricsLine& a, const LyricsLine& b) { return a.time_ms < b.time_ms; });
...
int Lyrics::GetCurrentLineIndex() const { return current_line_index_; }

int Lyrics::GetLineCount() const { return lines_.size(); }
...
bool Lyrics::HasLyrics() const { return !lines_.empty(); }